          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {related.map(r => (
              <Link key={r.id} to="/products/$productId" params={{ productId: r.id }} className="group bg-card rounded-xl border overflow-hidden hover:shadow-elegant hover:-translate-y-1 transition-all">
                <div className="aspect-[4/3] overflow-hidden bg-white"><ProductImage src={r.image} alt={r.name} className="w-full h-full" imgClassName="object-contain p-4 group-hover:scale-105 transition duration-700" /></div>
                <div className="p-5">
